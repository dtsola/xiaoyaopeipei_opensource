      ...
      >
        ...
        <div class="sku-image-area">
          ...
          <div class="sku-badges">
            <a-tag v-if="sku.status === 'inactive'" color="red">缺货</a-tag>
            <a-tag v-else-if="sku.status === 'active'" color="green">在售</a-tag>
            <a-tag
              :color="getDeviceColor(sku.device_type)"
              :style="{ backgroundColor: getDeviceBgColor(sku.device_type), border: 'none', color: '#fff' }"
            >
              {{ sku.device_type }}
            </a-tag>
          </div>
...
const deviceTabs = computed(() => {
  const stats = skuStore.summary?.device_type_stats || {}
  return [
    { key: 'all', label: '全部', count: skuStore.summary?.total || 0 },
    { key: 'desktop', label: '台式机', count: stats.desktop || 0 },
    { key: 'laptop', label: '笔记本', count: stats.laptop || 0 },
    { key: 'aio', label: '一体机', count: stats.aio || 0 }
  ]
})

// SKU列表（使用API数据）
const skuList = computed(() => skuStore.skuList)
...
// 获取设备类型背景色
function getDeviceBgColor(type: string) {
  const bgColors: Record<string, string> = {
    'laptop': 'rgba(24, 144, 255, 0.85)',
    'desktop': 'rgba(82, 196, 26, 0.85)',
    'aio': 'rgba(250, 140, 22, 0.85)'
  }
  return bgColors[type] || 'rgba(0, 0, 0, 0.65)'
}
...
function handleCopy(sku: SKU) {
  ...
  // 这里可以跳转到编辑页面并预填充数据
  setTimeout(() => {
    router.push(`/sku/add?copy=${sku.id}`)
  }, 500)
}
...
async function fetchSKUList() {
  try {
    await skuStore.fetchList({
      ...
      limit: 100,
      device_type: activeTab.value === 'all' ? undefined : activeTab.value,
