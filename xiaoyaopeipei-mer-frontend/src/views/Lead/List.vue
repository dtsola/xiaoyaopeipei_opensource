<template>
  <div class="lead-list-container">
    ...
    >
      <p class="call-info">
        手机号：<strong>{{ currentLead?.phone }}</strong>
        <a-button type="link" size="small" @click="copyPhone">
          <CopyOutlined />
          复制
        </a-button>
      </p>
...
const statusTabs = computed(() => [
  { key: 'all', label: '全部', count: leadStore.total },
  { key: 'pending', label: '待跟进', count: 0 },
  { key: 'contacted', label: '已联系', count: 0 },
  { key: 'closed', label: '已成交', count: 0 },
  { key: 'abandoned', label: '已放弃', count: 0 }
])

// 筛选后的线索列表
const filteredLeads = computed(() => {
  let result = [...leadStore.leadList]
...
async function handleStatusChange(leadId: string, newStatus: string) {
  try {
    await leadStore.updateStatus(leadId, newStatus)
...
function handleCall(lead: Lead) {
  if (lead.phone_full) {
    navigator.clipboard.writeText(lead.phone_full)
...
function handleCallConfirm() {
  ...
  message.success(`已打开联系方式：${currentLead.value?.phone}`)
}
...
// 复制电话号码
function copyPhone() {
  if (currentLead.value?.phone_full) {
    navigator.clipboard.writeText(currentLead.value.phone_full)
    message.success('电话号码已复制')
  }
}
...
async function fetchLeadList() {
  try {
    await leadStore.fetchList({
      page: currentPage.value,
      limit: 100,
      status: activeStatus.value === 'all' ? undefined : activeStatus.value,
      device_type: selectedDeviceTypes.value[0] || undefined,
...
// 监听筛选条件变化
watch([activeStatus, selectedDeviceTypes], () => {
  currentPage.value = 1
  fetchLeadList()
})
